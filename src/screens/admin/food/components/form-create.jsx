...
import useCreateFood from '../../../../service/hooks/admin/food/useCreate';
...
const model = Schema.Model({
  ...
  rangePriceFrom: Schema.Types.NumberType().isRequired('Price range from is required'),
  rangePriceTo: Schema.Types.NumberType().isRequired('Price range to is required'),
...
const FormCreateFood = () => {
  ...
  const { onCreate, isLoading } = useCreateFood();
  ...
  return (
    <Form className='min-w-[30rem] w-[40vw]' model={model}>
      ...
      <div className='flex justify-start items-center gap-10'>
        <Form.Group controlId='rangePriceFrom'>
          <Form.ControlLabel>{renderLabel('Price Range From')}</Form.ControlLabel>
          <Form.Control type='number' name='rangePriceFrom' style={{ width: '18vw' }} />
        </Form.Group>
        <Form.Group controlId='rangePriceTo' className='mb-5'>
          <Form.ControlLabel>{renderLabel('Price Range To')}</Form.ControlLabel>
          <Form.Control type='number' name='rangePriceTo' style={{ width: '18vw' }} />
        </Form.Group>
      </div>
